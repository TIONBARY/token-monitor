...
- **항상 위에 고정** — 핀 버튼(📌)으로 토글
- **드래그 이동** — 바와 상세 팝업 모두 원하는 위치로 자유롭게 이동
- **시스템 트레이 아이콘** — 실행 중임을 한눈에 확인, 우클릭으로 상세 보기 / 언어 설정 / 바 표시·숨기기 / 종료
...
|------|------|
| 바 더블클릭 | 상세 팝업 열기/닫기 |
| 바 우클릭 | 메뉴 (상세 보기 / 언어 설정 / 종료) |
| 바 또는 상세 팝업 제목 영역 드래그 | 창 위치 이동 |
| 📌 클릭 | 항상 위 고정 토글 |
| 트레이 아이콘 우클릭 | 메뉴 (상세 보기 / 언어 설정 / 바 표시·숨기기 / 종료) |

### 트레이 아이콘

작업표시줄 트레이에 작은 아이콘이 떠서 프로그램이 실행 중임을 바로 확인할 수 있고, 플로팅 바가 다른 창에 가려져 있어도 제어할 수 있습니다. 윈도우는 새로 추가된 트레이 아이콘을 "숨겨진 아이콘"(`^` 화살표) 영역에 넣는 경우가 많으니, 항상 보이게 하려면 작업표시줄로 드래그해서 꺼내두세요.
